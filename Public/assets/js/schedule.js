...
function fetchAvailableLessons(data = new FormData(), lessonsElement) {
    fetch("/ajax/getAvailableLessonsForSchedule", {
        method: "POST",
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
        },
        ...
        .then((data) => {
            ...
            data.forEach((lesson) => {
                out += `
                  <div class="d-flex justify-content-between align-items-start mb-2 p-2 rounded text-bg-primary">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function programChangeHandle() {
        ...
        if (scheduleTableElements.length < 1) {
            ...
            fetchAvailableLessons(data, availableItemElements[0])
            fetchScheduleTable(data, scheduleTableElements[0])
        } else {
            for (var i = 0; i < scheduleTableElements.length; i++) {
                ...
                fetchAvailableLessons(data, availableItemElements[i])
                fetchScheduleTable(data, scheduleTableElements[i])
            }
        }
    }
